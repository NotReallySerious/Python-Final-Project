...
def remove():
    try:
        ...
        rows = []
        removed_count = 0
        with open("medicine_stock.txt", "r", newline="") as file:
...
def prepare(p_id, t_id):
    ...
    with open("medicine_stock.txt", "r", newline="") as f1, \
         open("med_db.txt", "r", newline="") as f2:
        reader1 = csv.reader(f1)
        reader2 = csv.reader(f2)
        rows1 = [row for row in reader1]
        rows2 = [row for row in reader2]

    with open("doctor_prescription.txt", "r", newline="") as f3:
